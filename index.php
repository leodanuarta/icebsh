<?php
    ...
    $sql = "SELECT judul FROM nama_tabel WHERE kondisi = 'sesuai_kondisi'";
    $result = $conn->query($sql);
    
    if ($result->num_rows > 0) {
        // Output data dari setiap baris
        while($row = $result->fetch_assoc()) {
            // Tetapkan judul halaman sesuai data yang diambil
            $pageTitle = $row["judul"];
            break; // Hanya mengambil judul dari baris pertama saja
        }
    } else {
        $pageTitle = "Judul Default"; // Judul default jika tidak ada data yang ditemukan
    }
